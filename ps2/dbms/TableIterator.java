...
public class TableIterator {
    ...
    public boolean next() throws DeadlockException, DatabaseException {
        ...
        // cursor get next
        while (this.cursor.getNext(this.key, this.value, null) != OperationStatus.NOTFOUND) {
            // Only increment num_tuples if the WHERE clause isn't violated
            if (this.where.isTrue()) {
                this.numTuples++;
                return true;  
        ...
        // if there are no tuples left for the iterator to visit, return false
        return false;              
    }
    ...
    public Object getColumnVal(int colIndex) {
        ...
        if (this.value.getSize() == 0) {
            throw new IllegalStateException("the iterator has not yet been positioned on a tuple");
        }
        ...
        // read the record header for offset of offset
        int offsetIndex = colIndex * 2;
        // System.out.println(offsetIndex);
        // read the offset of value
        Short offset = valIn.readShortAtOffset(offsetIndex);
        // System.out.println((int)offset);
        // if offset = -1
        if (offset.equals((short)-1)) {
            return (Object)null;
        }
        // else if offset = -2
        else if (offset.equals((short)-2)) {
            // is primary key
            valIn = new RowInput(this.key.getData());
            // keyBuffer
            // System.out.println(type);
            // type is int
            if (type == 0) {
                // System.out.println(valIn.readIntAtOffset(0));
                return (Object)valIn.readIntAtOffset(0);
            }
            // type is double
            else if (type == 1) {
                // System.out.println(valIn.readDoubleAtOffsetDROP TABLE Course;(0));
                return (Object)valIn.readDoubleAtOffset(0);
            }
            // type is char or varchar
            else if (type == 2 || type == 3) {
                // System.out.println(this.key.getSize());
                return (Object)valIn.readBytesAtOffset(0, this.key.getSize());
            }
        }
        // else, a normal offset
        else {
            // System.out.println(offset);
            Short nextOffset = valIn.readNextShort();
            int nextOffsetIndex = (int)offsetIndex + 2;
            while (nextOffset == (short)-1 && offsetIndex <= this.table.numColumns() * 2) {
                nextOffset = valIn.readShortAtOffset(nextOffsetIndex);
                nextOffsetIndex += 2;
            }
            int length = 0;
            if (offset != (short)-1) {
            ...
            // type is int
                ...
                // System.out.println(valIn.readIntAtOffset((int)offset));
                return (Object)valIn.readIntAtOffset((int)offset);
            }
            // type is double
            else if (type == 1) {
                // System.out.println(valIn.readDoubleAtOffset((int)offset));
                return (Object)valIn.readDoubleAtOffset((int)offset);
            }
            // type is string
            else if (type == 2 || type == 3) {
                // System.out.println(valIn.readBytesAtOffset((int)offset, length));
                return (Object)valIn.readBytesAtOffset((int)offset, length);
            }
        }
        return (Object)null;
    }
