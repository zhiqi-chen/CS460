...
public class InsertStatement extends SQLStatement {
    ...
    public void execute() throws DatabaseException, DeadlockException {
        try {
            ...
            OperationStatus ret= db.putNoOverwrite(null, key, value);//how to use this
            if (ret == OperationStatus.KEYEXIST) {
                throw new Exception(key + ": an existing key/pair with this key " +
                                    "already exists");
                
            }else if(ret ==OperationStatus.SUCCESS){
                String name = table.getName();
                System.out.println("Added 1 row to " + name + ".");
            }
